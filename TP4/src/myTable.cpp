...
float difLid[3] = {0.8, 0.8, 0.8};
float specLid[3] = {0.05, 0.05, 0.05};
...
myTable::myTable() {
	...
	tableAppearance = new CGFappearance("textures/table.png",1,1);
	tableAppearance->setDiffuse(difLid);
	tableAppearance->setAmbient(ambLid);
	tableAppearance->setSpecular(specLid);
	tableAppearance->setShininess(shininessLid);
