...
myCylinder::myCylinder(int slices, int stacks, bool smooth) {
	...
	//double h = 0.5 / sin(angle/2.0);

	for(int i = 0; i < numVertex; i++) {
		vertX[i] = cos((angle/2.0) + i*angle);
		vertY[i] = sin((angle/2.0) + i*angle);
		std::cout << "vert " << i << " (" << vertX[i] << ", " << vertY[i] << ")" << std::endl;
	}
}
...
void myCylinder::draw() {
	...
	for(int i = 0; i < numVertex; i++) {
		...
		for(int j = 0; j < stacks; j++) {
			...
			//std::cout << "Normal " << i << " (" << cos((angle/2.0) + i*angle) << ", " << sin((angle/2.0) + i*angle) << ")" << std::endl;
			if(i == (numVertex - 1)) {
				if(smooth) {
